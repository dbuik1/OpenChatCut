...
export const EDIT_ITEM_TOOL_SCHEMAS: AgentToolSchema[] = [
  {
    name: 'edit_item',
    description:
      'Unified item-level operations across video, image, audio, gif, svg, motion-graphic, text, solid, adjustment, effect, and transition types. '
      + 'This is also how a clip is added to the timeline, whatever the user calls it: an intro, outro, end card, title card, logo, watermark, lower third, B-roll, cutaway or overlay is an add of the matching media, text or image item. '
      ...
      + 'adds place library items (effect/transition/zoom/MG/SFX), a POOL asset as a clip (type=video|image|gif|svg|audio, assetId=…), OR authored clips without assetId: type=text (text/fontSize/color/fontWeight/align?), type=solid (color?), or type=adjustment (an adjustment layer: grades every video track below it for its range with filters; placed on the topmost clear video track or a new one above; then set filters with an update). '
    ...
    input_schema: {
      ...
      properties: {
        ...
        updates: {
          type: 'array',
          description:
            'Generic clip update — type MUST be the item\'s actual kind, one of: video, audio, image, gif, svg, text, solid, adjustment, motion-graphic. NEVER write the literal "generic" or "clip" as type — they are rejected ("update type not supported"). Shape: {type,itemId|id,track|trackId?,fromFrame|startFrame?,durationInFrames?,srcInFrame|sourceStartFrame?,sourceDurationInFrames?,props?,volume?,fadeInSeconds?,fadeOutSeconds?,keyframes?,clearKeyframes?,filters?,transform?,backgroundFill?,backgroundFillStrength?,speed|playbackRate?}. Exact pool replacement: {type,itemId|id,assetId,sourceStartFrame?,sourceDurationInFrames?}; default replacement keeps the original timeline duration and resets the new source in-point to zero. Explicit slip: {operation:"slip",itemId|id,deltaInFrames}; positive moves the source window later, negative earlier, while timeline placement/duration stay fixed. The result reports appliedDeltaInFrames, srcInFrame, sourceWindow, clamped/status, or a structured code for unknown/invalid inputs. Roll: {operation:"roll",itemId|id,edge:"start"|"end",deltaInFrames} moves the cut at that edge together with the flush neighbour, so the timeline length never changes; positive moves the cut later. Slide: {operation:"slide",itemId|id,deltaInFrames} moves the clip in time keeping its content and duration while the flush neighbours before and after absorb the shift. Both clamp to every touched clip keeping one frame and to source handles, and report affected[] with each rewritten clip\'s timing. '
            ...
            + 'filters: {brightness?,contrast?,saturate? 0..2 (1=normal), blur? 0..30 px} on visual clips and adjustment layers (an adjustment layer applies them to every video track below it; its opacity/fades are the grade strength; effects[] and transform other than opacity are rejected on it). transform: {scale? 0.05..16, x?/y? % of canvas, rotation? deg, opacity? 0..1, borderRadius?, crop?|flexCrop?: {left?,right?,top?,bottom?} composition pixels}. Flex crop (flexcrop) = Crop Left/Right/Top/Bottom; left/right 0..canvas width px, top/bottom 0..canvas height px; cropped pixels are fully transparent; null clears. backgroundFill:true fills unused canvas with a blurred copy; backgroundFillStrength sets an exact integer percentage from 0 to 100 and enables the fill when used alone. Only video/image clips on the bottom video track (V1). speed/playbackRate: 0.1..8 on video/audio/gif (retimes duration). '
