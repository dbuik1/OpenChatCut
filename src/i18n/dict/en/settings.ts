...
export default {
  ...
  '调整整个编辑器的缩放比例（80%–500%）。桌面版保存后立即生效，也可用 Ctrl/Cmd + +/- 快速调整、Ctrl/Cmd + 0 复位。浏览器版请使用浏览器自带缩放。':
    'Adjust the entire editor scale (80%–500%). Desktop changes apply immediately after saving; Ctrl/Cmd + +/- adjusts quickly, and Ctrl/Cmd + 0 resets. In the browser, use the browser zoom controls.',
  '本地模型': 'Local models',
  ...
  '桌面版启动时检查本项目的新版本。': 'The desktop app checks for new releases of this project at launch.',
  '自动更新': 'Automatic updates',
  'Windows 与 Linux 桌面版每次启动都会检查更新。开启后发现新版本即在后台下载，下载完成后提示重启安装，不会自行重启。macOS 版请前往发布页下载。':
    'The Windows and Linux desktop apps check for updates at every launch. When on, a new version downloads in the background and you are asked to restart to install it; the app never restarts on its own. On macOS, download from the releases page.',
  '自动下载更新': 'Download updates automatically',
  '发现新版本即下载': 'Download as soon as one is found',
  '仅提示，手动下载': 'Notify only, download by hand',
  '诊断': 'Diagnostics',
