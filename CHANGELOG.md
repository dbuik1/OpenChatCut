...
- Two bundled skills for cutting gameplay and stream VODs with your own commentary. `gameplay-peaks` finds highlight candidates from the audio energy envelope rather than transcript content, so a clutch play carried by a shout and a game cue still surfaces where `find_highlights` — which selects on talking-head speech — returns nothing; it then cross-references each peak against the transcript, scene cuts and frames before proposing markers or sequences. `learn-my-fillers` counts one speaker's own verbal tics in their transcript, gets the list approved, and passes it to `clean_script` as `extraFillers`, which is the gap the fixed `um`/`uh` vocabulary leaves for a speaker whose crutch word is "like" or "basically". Both ship `scripts/`, so both depend on the materialisation above; neither applies anything without approval.
  新增两个内置技能，面向带自己解说的游戏与直播录像剪辑。`gameplay-peaks` 依据音频能量包络而非文字稿内容寻找高光候选：靠一声喊叫加游戏音效撑起来的高光时刻，`find_highlights`（按口播内容选取）找不到，它能找到；随后再用文字稿、场景切分与抽帧逐个交叉验证，才提出打点或切片建议。`learn-my-fillers` 从文字稿中统计该讲述者自己的口头禅，经用户确认后作为 `extraFillers` 传给 `clean_script`——固定的 `um`/`uh` 词表对口头禅是 "like"、"basically" 的人正好无能为力。两者都带 `scripts/`，因此都依赖上面的落盘机制；两者都不会未经确认就改动工程。
- A bundled skill that ships a `scripts/` directory now works. The root cause is that the two halves of the skill system read from different places: `src/agent/skills/<slug>/` is raw-imported into the renderer bundle by `import.meta.glob`, so `load_skill` returns every file as a string, while `run_skill_script` `execFile`s inside `~/.openchatcut/skills/<slug>/` on disk — a string in a JS bundle is not a directory, so such a skill was readable and permanently unrunnable, and any skill with executable logic had to be copied into the profile by hand. Every scripts-bearing bundled skill is now materialised whole (SKILL.md, `references/`, `scripts/`) into the skills root on first server start, honouring `OPENCHATCUT_SKILLS_DIR`. A directory that already exists for that slug is never overwritten or merged into — the user's own edits win, and a missing directory is the only trigger. The copy is staged and renamed into place, so an interrupted write leaves either nothing or a complete skill rather than a half-populated directory that would read as installed. Skills without `scripts/` are deliberately not copied: they already work as text, and an on-disk duplicate would only drift from the bundle.
  内置技能现在可以携带 `scripts/` 目录并真正运行。根因是技能系统的两半从不同位置读取：`src/agent/skills/<slug>/` 由 `import.meta.glob` 原样打进渲染进程包，`load_skill` 因此能把每个文件作为字符串取回；而 `run_skill_script` 是在磁盘上的 `~/.openchatcut/skills/<slug>/` 里 `execFile`——JS 包里的字符串不是目录，于是这类技能可读但永远不可运行，任何带可执行逻辑的技能都只能由用户手工拷进配置目录。现在每个带 `scripts/` 的内置技能会在服务首次启动时被完整落盘（SKILL.md、`references/`、`scripts/`），并遵循 `OPENCHATCUT_SKILLS_DIR`。同名目录若已存在，绝不覆盖也不合并——用户自己的修改优先，只有目录缺失才会触发写入。落盘先写入暂存目录再整体改名，因此中断的写入要么什么都不留，要么留下一个完整技能，而不会留下一个「看起来已安装」的半成品目录。没有 `scripts/` 的技能刻意不落盘：它们作为文本已经可用，磁盘副本只会与包内副本产生漂移。
...
- Five tools in the published tool catalogue described themselves in Chinese (six entries, as `search_fonts` appears in both the edit and ask lists), so a model that does not read Chinese — a Codex CLI session driving OpenChatCut in English, say — had no usable instruction for `manage_transcript`, `edit_captions`, `search_fonts`, `install_skill` or `run_skill_script`. The root cause is that a tool description is the model's ONLY channel for how a tool works: there is no fallback it can consult when the text is in a language it cannot read. The descriptions are now English, technical detail for technical detail; the Chinese parameter examples on `search_content` and `manage_versions` are English too, and a verify asserts the whole catalogue — descriptions and every `input_schema` property description — stays free of CJK text.
  已发布工具目录中有五个工具用中文描述自身（`search_fonts` 同时出现在 edit 与 ask 列表，共六条目录条目），因此不懂中文的模型（例如以英文驱动 OpenChatCut 的 Codex CLI 会话）拿不到 `manage_transcript`、`edit_captions`、`search_fonts`、`install_skill`、`run_skill_script` 的可用说明。根因在于工具描述是模型了解该工具的**唯一**渠道：文本语言读不懂时没有任何兜底可查。现在这些描述已逐条按技术细节译为英文，`search_content` 与 `manage_versions` 的中文参数示例也一并改为英文，并新增 verify 断言整个目录（含所有 `input_schema` 属性描述）不再包含中日韩文字。
